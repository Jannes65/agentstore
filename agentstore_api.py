...
def fix_jannes(db: Session = Depends(get_db)):
    from sqlalchemy import text
    try:
        db.execute(text("UPDATE builders SET nostr_pubkey = 'npub1ecxjlz0rrvn386w6h57e92stp6whgt0mhshg0nn90qt55g8ypxdqc7a8js', name = 'Jannes du Plooy' WHERE id = 'jannes65'"))
        db.commit()
        return {"status": "done"}
    except Exception as e:
        db.rollback()
